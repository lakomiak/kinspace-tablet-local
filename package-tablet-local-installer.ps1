...
try {
    if ($Release) {
        ...
        if ($Legacy) {
            ...
            $target = Join-Path $distDir "KinpilotTabletLocal-legacy-release.apk"
        } else {
            ...
            $target = Join-Path $distDir "KinpilotTabletLocal-release.apk"
        }
    } else {
        if ($Audit -and $Legacy) {
            ...
            $target = Join-Path $distDir "KinpilotTabletLocal-legacy-audit.apk"
        } elseif ($Audit) {
            ...
            $target = Join-Path $distDir "KinpilotTabletLocal-audit.apk"
        } elseif ($Legacy) {
            ...
            $target = Join-Path $distDir "KinpilotTabletLocal-legacy-debug.apk"
        } else {
            ...
            $target = Join-Path $distDir "KinpilotTabletLocal-debug.apk"
        }
    }
    ...
    if (!$Release -and !$Legacy -and !$Audit) {
        ...
        $provisionTarget = Join-Path $distDir "Provision-KinpilotTablet.ps1"
        Copy-Item -Force $provisionScript $provisionTarget
        $wipeTarget = Join-Path $distDir "Wipe-KinpilotTabletData.ps1"
        if ($IncludeWipeTool) {
    ...
    if (!$Release -and !$Legacy -and !$Audit) {
        ...
        Write-Host ".\dist\Provision-KinpilotTablet.ps1"
        Write-Host ""
        ...
        Write-Host ".\dist\Wipe-KinpilotTabletData.ps1 -Launch"
        }
    }
}
finally {
